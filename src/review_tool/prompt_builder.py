...
def build_system_prompt(
    ...
    parts.append(
        ...
        "When you can provide an exact code fix, include a code suggestion block "
        "immediately after the finding. The block MUST contain the exact replacement "
        "code for lines `line` through `endline` (the lines specified in the finding header). "
        "Use this format:\n"
        "```suggestion\n"
        "replacement code here\n"
        "```\n\n"
        "IMPORTANT rules for code suggestion blocks:\n"
        "- The suggestion replaces the ENTIRE line range (line through endline) from the finding\n"
        "- Include the complete corrected lines, not just the changed parts\n"
        "- Preserve indentation exactly as it appears in the original file\n"
        "- Only include suggestion blocks when you are confident in the exact fix\n"
        "- Do NOT include suggestion blocks for findings that need human judgment or design decisions\n\n"
